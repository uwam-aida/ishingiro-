    ...
    <div className="min-h-screen bg-[#FDFDFD] p-4 md:p-8 pb-20 space-y-10 max-w-7xl mx-auto">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
           <h2 className="text-lg font-bold text-red-600">Damaged Items (Store Keeper)</h2>
           <span className="bg-red-50 text-red-500 text-[10px] font-bold px-2 py-1 rounded-full border border-red-100">ATTENTION</span>
        </div>
        <p className="text-xs text-gray-400 -mt-3">Items returned by the store keeper due to quality issues.</p>
            ...
            <tbody className="divide-y divide-gray-5">
              {damagedItems.map((item, i) => (
                <tr key={i} className="hover:bg-gray-50/80 transition-colors">
                  ...
                  <td className="px-6 py-5 text-gray-500 text-sm uppercase">{item.reason}</td>
                  <td className="px-6 py-5 text-right">
                    <span className="bg-red-100 text-red-600 text-[10px] font-bold px-4 py-2 rounded-lg uppercase tracking-wide shadow-sm">
                      {item.status}
                    </span>
                  </td>
                </tr>
