...
import { Bell, Clock, ArrowLeft, CheckCircle2, Loader2, ShieldCheck, Users, Key, AlertCircle, LogOut } from 'lucide-react';
...
export default function MarketingManagerNotificationsPage() {
  ...
  const getNotificationIcon = (message: string) => {
    ...
    if (message.toLowerCase().includes('password') || message.toLowerCase().includes('code')) 
      return <Key size={18} />;
    if (message.toLowerCase().includes('security')) 
      return <ShieldCheck size={18} />;
    if (message.toLowerCase().includes('error') || message.toLowerCase().includes('alert')) 
      return <AlertCircle size={18} />;
    return <Bell size={18} />;
  };

  const getNotificationColor = (message: string) => {
    if (message.toLowerCase().includes('user')) return 'bg-purple-50 text-purple-600';
    if (message.toLowerCase().includes('password')) return 'bg-orange-50 text-orange-600';
    if (message.toLowerCase().includes('security')) return 'bg-blue-50 text-blue-600';
    if (message.toLowerCase().includes('alert')) return 'bg-red-50 text-red-600';
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
      ...
      <div className="flex items-center gap-4 pb-6 border-b border-gray-100">
        ...
        <div className="flex-1">
          ...
          <p className="text-[#F57C00] text-[10px] font-black uppercase tracking-widest">Marketing Manager Alerts</p>
        </div>
