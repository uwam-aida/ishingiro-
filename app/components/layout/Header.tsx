...
export default function Header({ 
  ...
  return (
    /* FIX: Changed z-index to 100 to ensure it stays above all body content.
        ...
        Changed px-8 to px-4 md:px-8 for mobile responsiveness.
    */
    <header className="h-20 bg-[#F6F6F6] px-4 md:px-8 flex items-center justify-between sticky top-0 z-[100] border-b border-gray-200 w-full shadow-sm">
      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        <div className="relative">
          <button 
            ...
            className="p-2 md:p-2.5 text-gray-500 hover:bg-white hover:shadow-sm rounded-xl transition-all relative group"
          >
            {/* ✅ FIXED: Added Bell Icon inside the button */}
            <Bell size={22} className="group-hover:text-[#F57C00] transition-colors" />
            
            {/* ✅ FIXED: Added logic for the notification unread badge */}
            {unreadCount > 0 && (
              <span className="absolute top-1.5 right-1.5 w-4 h-4 bg-red-500 text-white text-[10px] font-black flex items-center justify-center rounded-full border-2 border-[#F6F6F6]">
                {unreadCount}
              </span>
            )}
          </button>
        ...
        <div className="flex items-center gap-3 cursor-pointer group">
          ...
          <span className="text-xs md:text-sm font-bold text-gray-900 group-hover:text-[#F57C00] transition-colors uppercase tracking-tight">
