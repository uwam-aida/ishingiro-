...
import { Bell, Clock, ArrowLeft, CheckCircle2, Loader2, TrendingUp, AlertTriangle, Package, DollarSign } from 'lucide-react';
...
export default function FinanceNotificationsPage() {
  ...
  const getNotificationIcon = (message: string) => {
    ...
    if (message.toLowerCase().includes('damage') || message.toLowerCase().includes('loss')) 
  ...
  const getNotificationColor = (message: string) => {
    if (message.toLowerCase().includes('revenue')) return 'bg-green-50 text-green-600';
    if (message.toLowerCase().includes('damage')) return 'bg-red-50 text-red-600';
    if (message.toLowerCase().includes('profit')) return 'bg-emerald-50 text-emerald-600';
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
      ...
      <div className="flex items-center gap-4 pb-6 border-b border-gray-100">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest">Chief of Finance Alerts</p>
        </div>
