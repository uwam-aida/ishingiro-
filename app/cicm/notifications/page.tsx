...
import { Bell, Clock, ArrowLeft, CheckCircle2, Loader2, Package, Truck, Store, AlertTriangle, Cake } from 'lucide-react';
...
export default function CICMNotificationsPage() {
  ...
  const getNotificationIcon = (message: string) => {
    if (message.toLowerCase().includes('baked')) return <Package size={18} />;
    if (message.toLowerCase().includes('delivered')) return <Truck size={18} />;
    if (message.toLowerCase().includes('stock')) return <Store size={18} />;
    if (message.toLowerCase().includes('damage')) return <AlertTriangle size={18} />;
    if (message.toLowerCase().includes('cake')) return <Cake size={18} />;
    return <Bell size={18} />;
  };

  const getNotificationColor = (message: string) => {
    if (message.toLowerCase().includes('baked')) return 'bg-orange-50 text-orange-600';
    if (message.toLowerCase().includes('delivered')) return 'bg-blue-50 text-blue-600';
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
      ...
      <div className="flex items-center gap-4 pb-6 border-b border-gray-100">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-black uppercase tracking-tight text-[#5D4037]">Notifications</h1>
          <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest">CICM Audit Alerts</p>
        </div>
